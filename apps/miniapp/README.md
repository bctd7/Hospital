...
页面不直接调用 `uni.request`。Token、错误转换、刷新与退出集中维护；恢复本地会话后的首批请求、并发 401 和
Access Token 过期共享同一次刷新，旧请求不能再次覆盖新 Token。网络错误保留可恢复会话，Refresh 明确失效才统一
退出。服务层不弹窗、不跳转，页面层不按名称反查业务 ID。
