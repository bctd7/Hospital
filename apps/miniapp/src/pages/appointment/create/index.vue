...
async function chooseDepartment(id: string) {
  ...
  try {
    ...
    items.value = result;
    if (result[0]) await chooseItem(result[0].itemId);
  } catch (error) {
    if (generation === requestGeneration) errorMessage.value = messageOf(error, "检查项目加载失败");
  }
}

async function chooseItem(id: string) {
  const generation = ++requestGeneration;
  ...
  <view class="booking-page">
    ...
    <view class="cascade">
      ...
      <scroll-view scroll-y class="cascade__column cascade__column--rooms">
        ...
        <button v-for="room in rooms" :key="room.roomId" class="cascade-option cascade-option--room" :class="{ selected: selectedRoomId === room.roomId }" @tap="chooseRoom(room.roomId)">
          <text>{{ room.roomDisplayName }}</text><text class="cascade-option__check">{{ selectedRoomId === room.roomId ? '✓' : '›' }}</text>
        </button>
        <text v-if="selectedItem && !rooms.length" class="column-empty">本周暂无可预约房间</text>
      </scroll-view>
    </view>
