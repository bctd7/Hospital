...
import type { BookingOption, ExaminationItem, RoomExaminationItem } from "@/types/appointment";
...
const rooms = ref<RoomExaminationItem[]>([]);
...
const campuses = computed(() => {
  const values = new Map<string, DepartmentOption["campus"]>();
  departments.value.forEach((value) => values.set(value.campus.campusId, value.campus));
  return [...values.values()];
});
const selectedCampus = computed(() => campuses.value.find((value) => value.campusId === selectedCampusId.value));
const campusDepartments = computed(() => departments.value.filter((value) => value.campus.campusId === selectedCampusId.value));
const filteredDepartments = computed(() => {
  const keyword = departmentSearch.value.trim().toLowerCase();
  return keyword
    ? campusDepartments.value.filter((value) => value.department.name.toLowerCase().includes(keyword))
    : campusDepartments.value;
});
...
onMounted(async () => {
  ...
  try {
    ...
    if (first) {
      selectedCampusId.value = first.campus.campusId;
      await chooseDepartment(first.department.departmentId);
    }
  } catch (error) {
...
function switchCampus() {
  if (campuses.value.length <= 1) return;
  uni.showActionSheet({
    itemList: campuses.value.map((value) => value.name),
    success: (result) => {
      ...
      const first = departments.value.find((value) => value.campus.campusId === campus.campusId);
      if (first) void chooseDepartment(first.department.departmentId);
    },
  });
}
...
async function chooseItem(id: string, revealRooms = true) {
  ...
  try {
    const [availableRooms, result] = await Promise.all([
      patientAppointmentApi.listItemRooms(id),
      patientAppointmentApi.listBookingOptions(id),
    ]);
    if (generation !== requestGeneration) return;
...
function hasBookableOptions(roomId: string) {
  return options.value.some((value) => value.roomId === roomId);
}
...
<template>
  <view class="booking-page">
    <view class="search-bar"><text class="search-bar__icon">⌕</text><input :value="departmentSearch" confirm-type="search" placeholder="搜索当前院区科室" @input="updateSearch" /></view>
    ...
    <view class="hospital-card">
      ...
      <view class="hospital-card__content">
        <text class="hospital-card__name">{{ selectedCampus?.name || '医院院区' }}</text>
        <text class="hospital-card__level">检查预约</text>
        <text class="hospital-card__address">{{ selectedDepartment?.department.name || '请选择科室' }}</text>
      </view>
      <button v-if="campuses.length > 1" class="hospital-card__switch" @tap="switchCampus">切换院区⌄</button>
    </view>
      ...
      <view class="workspace-track" :class="workspaceClass">
        ...
        <view class="workspace-column workspace-column--items">
          ...
          <scroll-view scroll-y class="column-body">
            ...
            <button v-for="item in items" :key="item.itemId" class="cascade-option" :class="{ selected: selectedItemId === item.itemId }" @tap="chooseItem(item.itemId)">
              <view class="item-summary"><text class="item-summary__name">{{ item.name }}</text><text class="item-summary__duration">{{ formatEstimatedDuration(item.estimatedDurationMinutes) }}</text></view><text class="cascade-option__arrow">›</text>
            </button>
        ...
        <view class="workspace-column workspace-column--rooms">
          ...
          <scroll-view scroll-y class="column-body">
            ...
            <button v-for="room in rooms" :key="room.roomId" class="room-card" :class="{ 'room-card--selected': selectedRoomId === room.roomId, 'room-card--unavailable': !hasBookableOptions(room.roomId) }" @tap="chooseRoom(room.roomId)">
              <text class="room-card__number">{{ room.roomNumber }}室</text>
              <text v-if="!hasBookableOptions(room.roomId)" class="room-card__availability">本周无可预约时段</text>
            ...
            <text v-if="selectedItem && !loadingOptions && !rooms.length" class="column-empty">该项目尚未配置可执行房间</text>
          </scroll-view>
        </view>
      </view>
    </view>
    ...
    <view v-if="showingItemRooms && selectedItem && selectedRoom" class="selection-detail">
      ...
      <text class="selection-detail__duration">{{ formatEstimatedDuration(selectedItem.estimatedDurationMinutes) }}</text>
      ...
      <text v-if="!roomOptions.length" class="column-empty">该房间已配置，但本周暂无可预约时段</text>
    </view>
