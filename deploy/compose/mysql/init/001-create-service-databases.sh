...
: "${GUIDANCE_MYSQL_USER:?GUIDANCE_MYSQL_USER is required}"
: "${GUIDANCE_MYSQL_PASSWORD:?GUIDANCE_MYSQL_PASSWORD is required}"
...
CREATE DATABASE IF NOT EXISTS hospital_guidance
    CHARACTER SET utf8mb4
    COLLATE utf8mb4_0900_ai_ci;
CREATE USER IF NOT EXISTS '${GUIDANCE_MYSQL_USER}'@'%'
    IDENTIFIED BY '${GUIDANCE_MYSQL_PASSWORD}';
GRANT ALL PRIVILEGES ON hospital_guidance.* TO '${GUIDANCE_MYSQL_USER}'@'%';
FLUSH PRIVILEGES;
EOSQL
)
