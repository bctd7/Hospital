...
- 当前确定使用 uni-app + Vue 3 + TypeScript + Vite 开发小程序，并编译为微信小程序代码交给微信开发者工具运行和调试；
- 首版只交付微信小程序，不因框架具备跨端能力而同时维护 H5 或独立 App；
- 一级导航确定为首页、挂号、消息、我的，第一阶段使用标准 TabBar；
- 后续如果发布 H5 或独立 App，需要单独验证页面、权限、隐私、支付和平台能力，不能把“可以编译”等同于“无需适配”。

前端外壳、目录边界和首阶段验收标准见 [`plan/miniapp/`](./miniapp/)。
...
1. 患者端和工作人员端具体登录方式；
2. 多项目预约的时段选择和全部成功/部分成功规则；
3. 哪些数据属于敏感数据以及保留期限；
4. Kafka 第一条领域事件及其生产者、消费者是什么；
5. 生产环境采用云服务器、托管容器还是容器编排平台；
6. 可用性、响应时间、恢复时间和数据恢复点目标。
