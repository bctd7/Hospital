...
1. **超级管理员**：管理平台、组织、部门、医生账号及跨部门业务数据。
...
组织结构调整是超级管理员专属能力，不向部门医生开放。首期至少包括两类业务：部门生命周期管理和医生账号生命周期管理。
...
- 启用或停用部门；
- 查看部门下的医生数量和业务关联情况；
- 为未来的医院、院区和上级部门关系预留组织层级。

已经被医生、预约、方案或报告引用的部门不应物理删除。部门停止使用时优先采用“停用”，历史业务继续保留原部门 ID 和当时的部门信息。

部门名称可能变化，因此业务数据应关联稳定的部门 ID，而不是把部门名称作为权限判断依据。
...
新增医生时需要避免为同一人员重复创建账号。具体使用工号、手机号、医院人员标识或其他方式识别医生，后续结合客户数据来源确定。

### 5.3 医生部门迁移

部门迁移应作为一项明确的业务操作，而不是直接修改数据库字段。一次迁移至少需要表达：
...
首期迁移可以立即生效：医生对目标部门获得全部业务权限，对原部门立即变为只读。历史业务数据保持原部门归属，不随医生迁移。

迁移、禁用和角色变更后，需要使旧授权在合理时间内失效。具体采用短期 Token、授权版本或缓存失效机制，在实施 Identity Service 时确定。

### 5.4 为后续组织调整预留空间

首期只实现新增、修改、停用部门以及医生单部门迁移，但数据和操作边界应允许以后增加：
...
这些扩展不进入首期，但不能通过写死两个部门名称或把部门直接绑定在业务代码中阻断未来演进。
...
organization.department.read / organization.department.create / organization.department.update / organization.department.disable
identity.doctor.read / identity.doctor.create / identity.doctor.update / identity.doctor.disable / identity.doctor.transfer
```
...
12. 新增医生时使用工号、手机号还是医院人员 ID 避免重复账号；
13. 部门停用前是否要求先迁出全部在职医生；
14. 部门更名是否需要在历史页面展示当时名称；
15. 医生调岗立即生效，还是未来需要支持指定生效时间；
16. 部门合并和拆分时，历史数据应继续归属旧部门还是建立新的映射关系。
