...
> 状态：收尾调整已实现（2026-08-15）；代码、契约和前端构建已通过，真实 MySQL 集成用例等待本地数据库进程可用后执行。
>
...
工作人员：查询本科室预约 -> 查看预约 -> 在项目时间内开始检查 -> 删除待检查预约
系统：占用/释放具体日期容量 -> 配置变化联动删除 -> 将超时待检查预约标记为未到场
```
...
- 工作人员开始检查后释放患者的 session 占位，患者可以立即预约该日期同一 session 的下一个项目；房间容量仍按原规则独立统计；
...
- 项目结束后仍为 `confirmed` 的预约自动转为 `no_show`，保留记录并释放容量及患者 session 占位；
- `in_progress` 表示检查已经开始，即使项目时间结束也不自动完成或转为未到场；
- 项目结束后禁止工作人员再把 `confirmed` 预约改为 `in_progress`；
- 每名患者每个自然周最多成功创建 10 次预约；只有预约事务成功提交才消耗一次额度，取消、完成和失约均不返还；
...
- `confirmed` 是待检查，`in_progress` 是检查中，`completed` 是已完成，`no_show` 是未到场；
- 状态只允许 `confirmed -> in_progress -> completed` 或由系统执行 `confirmed -> no_show`；
- 预约不允许修改项目、房间、日期或 session；
- 患者 ID 从认证上下文取得，请求不能替患者伪造；
- 开始和完成检查保存实际操作者账号，但不把操作者变成预约资源。
...
预约写操作继续使用 UUID `operation_id`。创建、患者删除、工作人员删除和开始检查都必须幂等；相同
...
取得占位；患者删除、工作人员删除、配置联动删除、开始检查和转为未到场时释放占位。
该表是事务约束，不是管理员配置，也不进入热点缓存。

### 3.5 患者自然周额度

`appointment_patient_weekly_quota_usage` 以
`PRIMARY KEY(patient_account_id, week_start_date)` 保存患者在一个自然周已经成功创建的预约次数。该表属于
Appointment 自身的预约约束，不调用 Identity；`patient_account_id` 来自认证上下文，只作为稳定逻辑标识。

- 初始上限固定为 10；
- 创建预约在同一 MySQL 事务内原子执行 `used_count + 1 <= 10`；
- 后续容量、session 占位或预约插入失败时，整个事务回滚，不消耗额度；
- 预约成功后，无论患者取消、工作人员删除、完成检查或最终失约，均不减少 `used_count`；
- 新自然周使用新的 `week_start_date` 行，不运行统一重置任务。
...
POST   /api/v1/admin/appointment/bookings/:bookingId/start-examination
...
4. 原子消耗当前自然周的一次额度，保证 `used_count + 1 <= 10`；
5. 锁定并校验项目、房间、关系和两条周窗口；
6. 校验当前时间没有超过目标日期的项目停止新增时间；
7. 创建或锁定 `(room_id, service_date, session)` 容量行；
8. 原子执行 `occupied_capacity + 1 <= total_capacity`；
9. 插入 `confirmed` 预约及时间快照；
10. 写入幂等操作记录；
11. 提交后失效本周候选和本人预约缓存。
...
增加 Appointment 内部定时任务，按医院时区处理项目结束时间已经到达且仍为 `confirmed` 的预约：

- 查询条件使用 `(service_date, status)` 索引；
- 小批量、可重试处理，不执行全表大事务；
- 每条预约转为 `no_show`，保留预约记录，并且最多释放一次容量和患者 session 占位；
- 服务重启后会补扫遗漏日期；
- `in_progress` 表示检查已经实际开始，不因项目窗口结束而自动终止或转为失约。
...
### 10.1 进行中预约与诊后记录的用户视图

预约状态仍由同一张 `appointment_bookings` 表和同一套状态机维护，不新增“历史预约”表，也不复制完成数据。
面向患者和工作人员的列表读取统一分为两个互斥视图：

- `active`：只包含 `confirmed`、`in_progress`，用于诊前“我的预约 / 科室预约”；
- `completed`：包含 `completed`、`no_show`，用于诊后“检查记录”。

完成状态在用户界面中是终态：患者、医生和管理员均不获得“退回待检查”“重新开始”等逆向操作。后台若因运维、
数据修复需要保留状态纠正能力，也不得通过普通用户接口或页面暴露。两个视图必须由服务端分页过滤，不能先拉取
全量预约再由前端隐藏，否则历史数据增长后仍会造成性能和分页错误。
...
5. 实现 staff manager 的列表、详情、开始检查和删除；
6. 把资源配置写操作接入预约联动；
7. 实现自动清理任务；
...
- 不同日期的容量和占用互不影响；
- 重复创建/删除/开始检查不会重复占用或释放容量；
- 配置关闭和容量降低按照已确认规则删除预约；
- 项目结束后仍待检查的预约会转为未到场，检查中的预约保持不变；
- 每周第 11 次成功预约被拒绝，取消预约不会恢复额度，新自然周自动使用新的额度；
...
以及患者创建/查询、医生按科室查询/开始检查/删除的端到端操作。物理手机自动化需要开发者工具提供测试账号
...
3. **已经开始检查后遇到配置修改**：不自动删除 `in_progress` 预约，存在冲突时拒绝配置修改；
4. **固定预约额度**：每名患者每个自然周最多成功创建 10 次预约；取消、完成和失约均不返还当周额度，下一周使用新的额度记录。

## 13. 本次收尾实施计划

1. 收口 Schema、Proto、API 和领域状态，完全移除 `checked_in` 与核销接口；
2. 让“开始检查”直接执行 `confirmed -> in_progress`，并在后端校验当前时间必须完整处于项目日期与时间窗口内；
3. 将定时清理改为按 `service_date + item_end_time_snapshot` 扫描，把超时 `confirmed` 原子转为 `no_show`，释放容量和 session 占位但不删除预约；
4. 增加 Appointment 本地自然周额度表，在创建预约事务中原子消耗额度，固定上限 10，任何删除或终态都不返还；
5. 统一患者端和工作人员端的活动/诊后视图、状态文案、按钮和错误提示；`no_show` 只读且不能填写报告；
6. 更新迁移、接口文档与测试，依次执行 Go 测试和 vet、API 校验、TypeScript、前端测试及微信小程序构建。
