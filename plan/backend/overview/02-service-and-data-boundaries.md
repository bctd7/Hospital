...
2. **Appointment Service**：检查项目、固态时间约束、窗口、活动容量、独立预约、建议顺序、叫号和检查执行；
3. **Navigation Service**：建筑、楼层、地点、地图版本和路线。

`app-api` 是微信小程序唯一 HTTP 接入层，负责协议适配和页面聚合，不拥有核心业务事实。

当前不建立 Planning Service。每个检查项目形成一条独立预约和执行链路；Appointment 内部根据固态时间
约束、科室窗口和地图耗时计算建议执行顺序，但顺序不是跨预约事务，不自动创建、取消或修改预约。

Identity 是身份事实服务，不是集中权限裁决服务。Appointment、Navigation、未来 Report 等服务在本地验证
...
| 检查项目、准备动作、固态时间约束及规则版本 | Appointment Service |
| 项目与院区、科室、地点关系 | Appointment Service |
| 到院窗口、停止新增时间和活动容量 | Appointment Service |
| 独立预约、建议顺序、报到、队列、叫号和检查执行 | Appointment Service |
| 建筑、楼层、地点、地图和路线 | Navigation Service |
...
- 检查项目目录，以及患者预约前需要了解的基础条件；
- 项目可由哪个院区、科室和地点执行；
- 科室分别配置的上午/下午具体时间、停止新增时间和活动容量；
- 一次一个检查项目的独立预约；
- 固态事件触发时间约束图和可替换求解器的建议顺序计算；
- 预约限制、处罚事件和限制状态；
- 取消、报到、排队、叫号、过号、失约、开始和完成；
- 患者取消、医院取消、失约和完成后的容量释放；
- 幂等、并发、状态历史、业务审计和 Outbox。

Appointment 不修改 Identity 的科室，不拥有地图结构，不保存报告正文，也不把建议顺序当作跨预约事务。
...
Identity 拥有医院、院区和科室；Navigation 拥有建筑、楼层、房间、地点、地图版本和路径。Appointment
保存 `location_id` 和预约发生时的地点摘要，不复制并反向维护地图结构。

首个 Appointment 闭环只要求稳定地点 ID 和最小地点摘要；楼层图、路线和室内定位按后续地图交付深度实现。
...
1. 填写方案 A 的额度参数，并确认固态时间约束、建议顺序目标、叫号和报告规则；
2. 补齐最小本人 ID 和地点查询能力；
3. 实现检查目录、科室窗口和活动容量；
4. 实现独立预约、取消和处罚事实；
5. 实现固态时间约束和经基准选型的建议顺序求解器；
6. 实现报到、队列、叫号、检查开始和完成；
7. 实现报告版本和患者查询；
8. 接入地图、通知和真实医院系统。
