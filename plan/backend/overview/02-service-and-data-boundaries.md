...
2. **Appointment Service**：检查项目、检查房间、独立周配置、活动容量、独立预约、建议顺序、叫号、
   检查执行和检查报告。
...
Identity 是身份事实服务，不是集中权限裁决服务。Appointment 等业务服务在本地验证
...
| 报告正文、版本、首版发布和更正 | Appointment Service 内部 Report 领域 |
...
- 报告草稿、首版原子发布、不可覆盖的更正版本和患者本人查询；
- 幂等、并发、状态历史、业务审计和 Outbox。

Appointment 不修改 Identity 的科室，也不把建议顺序当作跨预约事务。
...
报告作为 Appointment 内部领域交付，不建立独立 Report Service。一次预约对应一次检查和一个报告主体；草稿
可以编辑，首版正式报告必须与检查完成、容量释放在 Appointment 本地同一事务提交。正式版本不可覆盖，更正
必须填写原因并新增版本，患者只读取本人当前正式版本。

首期正文固定为检查所见、检查结论、处理建议和备注。附件、影像、OSS、PDF、多人审核、电子签章和撤回审批
不在当前实现范围；这些能力真正形成独立安全或存储边界后，再评估是否拆服务，不能提前建立空服务。
...
flowchart LR
    ...
    API --> APPT["Appointment Service / 内部 Report 领域"]
