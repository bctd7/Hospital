...
确认固定触发事件、`forbid/allow/not_before/not_after` 四种时间约束、规则发布和患者确认流程，以及 DFS
...
填写已选惩罚方案 A 的额度参数，确认固态时间约束、DFS 优化目标、叫号、报告和本人 ID 规则。
...
实现固定事件约束求值、时间区间计算、地图耗时输入、DFS/分支限界，以及可解释且可复现的建议顺序。
