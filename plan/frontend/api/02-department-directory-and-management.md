...
- 患者、医生和超级管理员登录后读取相同的有效医院目录，未登录请求返回 `401`；
- 超级管理员使用独立管理员接口维护院区和科室；
- 科室必须直接属于院区，首期不支持子科室；
- 开发 Mock 只能位于可替换 API 适配层并保持相同响应结构。
...
共享目录接口必须携带有效 Access Token，但不要求工作人员角色或额外 permission，并且必须在网关限流。
数据结构固定为：
...
| 场景 | 状态/业务码 | 前端行为 |
|---|---|---|
| 未登录、Access Token 过期或授权版本无效 | `401` | 进入统一刷新流程，最多重试一次；刷新被拒绝则回到登录页 |
...
- 未登录请求不能读取目录，普通登录用户不需要工作人员角色或额外 permission；
