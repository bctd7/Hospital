...
func (l *DeleteMyBookingLogic) DeleteMyBooking(req *types.DeleteBookingAPIRequest) (resp *types.DeleteBookingAPIResponse, err error) {
	rpcCtx, requestID, err := bookingRPCContext(l.ctx, l.svcCtx)
	if err != nil {
		return nil, err
	}
	value, err := l.svcCtx.Appointment.DeleteMyBooking(rpcCtx, &appointmentv1.DeleteBookingRequest{BookingId: req.BookingID, OperationId: req.OperationID, Reason: req.Reason, RequestId: requestID})
	if err != nil {
		return nil, err
	}
