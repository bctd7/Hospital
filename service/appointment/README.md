# Appointment Service

Appointment 服务当前实现三组能力：

- 检查项目目录：一个项目只属于一个科室；
- 科室资源配置：一个房间只属于一个科室，房间与本部门检查项目可多对多关联；
- 独立周配置：房间维护开放时间和共享容量，项目维护预约开始、停止新增和结束时间；
- 严格窗口校验：启用关联后，项目窗口必须完整落在对应房间同星期、同上午/下午窗口内；
- 患者查询：按检查项目返回可选择的有效房间，不做医生绑定或后端自动分配。
- 患者预约：查看本周候选、创建预约、查看本人预约和在项目开始前删除本人预约；
- 工作人员操作：医生和管理员按科室查看预约、在项目时间内开始检查和管理删除，不作为预约资源参与容量模型；
- 检查与报告：待检查预约直接开始检查；完成检查、发布首版报告和释放容量原子提交；正式报告通过新增版本更正，绝不覆盖旧版；
- 报告读取：工作人员按科室查看草稿和版本历史，患者只查看本人当前有效正式版本；
- 容量控制：按“房间 + 具体日期 + 上午/下午”独立计数，所有项目共享房间容量，每条预约占一个单位。
- 患者重复预约控制：同一患者在同一具体日期、同一上午/下午大窗只允许一条待检查预约；开始检查、删除或转为未到场后释放患者占位。
- 每周额度：每名患者每个自然周最多成功预约 10 次；取消、完成和失约均不返还额度，新周自然使用新记录。
- 身份能力继承：医生和管理员在工作人员端拥有管理能力，切换到患者端后同时拥有完整患者预约能力；预约仍记录当前账号自身的 `account_id`。

## 热点缓存

资源读取使用 Redis Cache Aside，MySQL 始终是最终数据源：

- 房间、房间项目关系、项目可选房间、房间周窗口、项目周窗口缓存 5 分钟并增加 0–20% 随机抖动；
- 患者侧本周预约候选查询使用 20 秒短缓存，未找到的单实体缓存 10 秒；
- 同一实例对相同缓存键合并并发回源，降低热点失效时的数据库冲击；
- 列表键包含科室 generation。写事务提交后递增 generation，新请求立即使用新键，无需扫描或批量删除旧键；
- Redis 读写失败自动回源 MySQL；后续预约与容量扣减必须在 MySQL 事务中复核，缓存不能作为容量授权依据。
- 创建、删除、开始检查、未到场处理和配置联动均在 MySQL 事务中锁定具体日期容量；Redis 即使短暂陈旧也只影响展示，不会造成超卖。

Redis 键统一使用配置的 `AppointmentRedis.Prefix`，资源模块再追加 `resource:` 命名空间。

## 验证

```powershell
go test ./service/appointment/...
go test ./service/app/api/...
goctl api validate -api contracts/api/app.api
```
