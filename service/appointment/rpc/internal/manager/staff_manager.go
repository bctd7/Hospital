package manager

import (
	"errors"
)

// StaffManager is the only appointment entry point for administrators and
// authorized department staff. Project, room and schedule operations are
// split into focused files but remain methods of this type.
type StaffManager struct {
	projectStore ProjectStore
	store        RoomScheduleStore
	cache        Cache
	flights      flightGroup
}

func NewStaffManager(projectStore ProjectStore, roomScheduleStore RoomScheduleStore, cache Cache) (*StaffManager, error) {
	if projectStore == nil {
		return nil, errors.New("appointment project store is required")
	}
	if roomScheduleStore == nil {
		return nil, errors.New("appointment room and schedule store is required")
	}
	return &StaffManager{projectStore: projectStore, store: roomScheduleStore, cache: cache}, nil
}
