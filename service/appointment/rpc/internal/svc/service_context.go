...
func NewServiceContext(c config.Config) (*ServiceContext, error) {
	...
	staffManager, err := staffmanager.NewManager(store, store, store, store, store, appointmentCache)
	if err != nil {
		appointmentRedisClient.Close()
		authorizationRedisClient.Close()
		store.Close()
	...
	patientManager, err := patientmanager.NewManager(store, store, store, store, appointmentCache)
	if err != nil {
		appointmentRedisClient.Close()
		authorizationRedisClient.Close()
		store.Close()
