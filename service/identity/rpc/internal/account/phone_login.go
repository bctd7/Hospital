...
type PhoneLoginManager struct {
	...
	provider     login.PhoneVerificationProvider
	sessions     SessionStarter
	phoneKey     []byte
	retrySeconds int64
}

func NewPhoneLoginManager(store PhoneLoginStore, provider login.PhoneVerificationProvider, sessions SessionStarter, phoneKey []byte, retrySeconds int64) (*PhoneLoginManager, error) {
...
func (m *PhoneLoginManager) Login(ctx context.Context, rawPhone, rawCode string) (session.TokenPair, error) {
	...
	if !verificationCode.MatchString(code) {
		return session.TokenPair{}, login.ErrInvalidCredential
	}
