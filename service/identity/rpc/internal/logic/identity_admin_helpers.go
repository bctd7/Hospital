...
func identityDirectoryRPCError(err error) error {
	if errors.Is(err, identityadmin.ErrNotFound) {
		return organizationStatusError(codes.NotFound, "department not found", "DEPARTMENT_NOT_FOUND")
	}
	return identityAdminRPCError(err)
}

func accountDisplayProfileResponse(value identityadmin.DisplayProfile) *identityv1.AccountDisplayProfile {
...
func doctorsPageResponse(value identityadmin.DoctorPage) *identityv1.ListDoctorsByDepartmentResponse {
...
func adminAccountSummaryResponse(value identityadmin.Account) *identityv1.AdminAccountSummary {
	departmentID := value.DepartmentID
	departmentName := value.DepartmentName
	if value.StaffStatus != identityadmin.StaffStatusActive {
		departmentID = ""
		departmentName = ""
	}
...
func adminAccountDetailResponse(value identityadmin.Account, actions []string) *identityv1.AdminAccountDetail {
...
func adminAccountsPageResponse(value identityadmin.AccountPage) *identityv1.ListAdminAccountsResponse {
