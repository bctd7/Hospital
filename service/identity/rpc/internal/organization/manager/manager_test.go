...
type Unit = organization.Unit
type UnitType = organization.UnitType
type Status = organization.Status
type ListFilter = organization.ListFilter
type Operation = organization.Operation
type Change = organization.Change
type TxStore = organization.TxStore

const (
	UnitTypeHospital   = organization.UnitTypeHospital
	UnitTypeCampus     = organization.UnitTypeCampus
	UnitTypeDepartment = organization.UnitTypeDepartment
	StatusActive       = organization.StatusActive
	StatusDisabled     = organization.StatusDisabled
)

var (
	ErrInvalid          = organization.ErrInvalid
	ErrForbidden        = organization.ErrForbidden
	ErrNotFound         = organization.ErrNotFound
	ErrConflict         = organization.ErrConflict
	ErrVersionConflict  = organization.ErrVersionConflict
	ErrInvalidHierarchy = organization.ErrInvalidHierarchy
	ErrActiveChildren   = organization.ErrActiveChildren
	ErrActiveDoctors    = organization.ErrActiveDoctors
)
...
func TestCreateUnitRequiresPermission(t *testing.T) {
	manager := NewUnitManager(newFakeOrganizationStore())
...
func TestGetDirectoryContextReturnsOnlyActiveCampuses(t *testing.T) {
	...
	manager := NewDirectoryManager(store)
...
func TestListDirectoryDepartmentsIncludesAuthoritativeCampus(t *testing.T) {
	manager := NewDirectoryManager(newFakeOrganizationStore())
